...
namespace BeatThat.UI
{
	...
	/// </summary>
	public class PlaceActiveWindowAboveScrim : MonoBehaviour, PanelDisplayController 
	{
		...
		public void UpdateDisplayList(IEnumerable<Transform> windowStack)
		{
			...
			if(finalStack[finalStack.Count - 1].GetComponent<DisplayWithoutScrim>() == null) {
				// assuming we have a scrim, put it right behind the new active window
				Transform myScrim = this.scrim;
				if(myScrim != null) {
					finalStack.Insert(finalStack.Count - 1, myScrim);
				}
